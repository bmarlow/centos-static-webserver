...
COPY create_index.sh /run/create_index.sh
#Make script executable
RUN chmod +x /run/create_index.sh
#Create index file ahead of time
RUN touch /run/index.html
...
CMD cd /run && bash create_index.sh && python -m SimpleHTTPServer 8080
